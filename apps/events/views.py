...
Gestion des événements, co-organisateurs et tables.
"""
...
from django.views.generic import CreateView, DetailView, ListView, UpdateView, View, DeleteView
...
from apps.guests.services import TableAssignmentService, generate_tables_pdf
...
def user_can_manage_event(user, event):
    ...
    Args:
        ...
        event: Evenement concerne
    
    Returns:
        bool: True si l'utilisateur est organisateur ou co-organisateur accepte
    """
...
class EventListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        """Recupere les evenements de l'utilisateur."""
...
class EventCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        """
        Sauvegarde l'evenement avec l'organisateur principal et applique le plan gratuit.
        """
        ...
        # Appliquer le plan gratuit par defaut
        event.selected_plan = Event.PlanChoices.FREE
        event.apply_plan_limits()
        
        event.save()
        
        # Creer une table par defaut si la gestion des tables est activee
        if event.has_tables:
...
class EventUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        """
        Sauvegarde l'evenement et gere l'activation/desactivation des tables.
        """
        event = form.save(commit=False)
        ...
        # Supprimer les tables si la gestion est desactivee
        ...
        # Creer une table par defaut si la gestion est activee
...
class EventDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    Inclut les statistiques, les reponses, les collaborateurs, les tables
    et les informations sur le plan et le paiement.
    """
    model = Event
    ...
    def get_context_data(self, **kwargs):
        ...
        verified_count = responses.filter(verification_status='verified').count()
        ...
        attendance_rate = 0
        if verified_count > 0:
            attendance_rate = round((attending_verified / verified_count) * 100, 1)

        # Nombre de personnes attendues
        expected_guests = 0
        for response in responses.filter(will_attend=True, verification_status='verified'):
            expected_guests += response.number_of_guests
        ...
        can_add_guests, guest_limit_error = event.can_add_guests()
        can_add_collaborator, collaborator_limit_error = event.can_add_collaborator()

        context.update({
            'recent_responses': responses.order_by('-submitted_at')[:10],
            'collaborators': event.collaborators.filter(
                status='accepted'
            ).select_related('user'),
            ...
            # Informations sur le plan et les limites
            'plan_info': event.get_plan_display_info(),
            'current_guest_count': total_invited,
            'max_guests_allowed': event.max_guests_allowed,
            'max_collaborators_allowed': event.max_collaborators_allowed,
            'remaining_guests': event.max_guests_allowed - total_invited,
            'can_add_guests': can_add_guests,
            'guest_limit_error': guest_limit_error,
            'can_add_collaborator': can_add_collaborator,
            'collaborator_limit_error': collaborator_limit_error,
            'is_paid': event.is_paid,
            'selected_plan': event.selected_plan,
            'has_payment_pending': event.has_payment_pending(),
            'has_payment_rejected': event.has_payment_rejected(),
            'payment_proof_image': event.payment_proof_image,
            'payment_rejection_reason': event.payment_rejection_reason,
            'payment_requested_at': event.payment_requested_at,
            'payment_verified_at': event.payment_verified_at,
        })
        return context
...
class EventDeleteView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        event = get_object_or_404(
            ...
            main_organizer=request.user
        )
        name = event.name
        event.soft_delete()
        messages.success(
            request,
            _('L\'evenement "%(name)s" a ete supprime.') % {'name': name}
        )
...
class JoinCoOrganizerView(View):
    ...
    Verifie que la limite de co-organisateurs n'est pas atteinte.
    """
    def get(self, request, slug, token):
        event = get_object_or_404(Event, slug=slug, coorganizer_token=token)
        
        if request.user.is_authenticated:
            # Verifier la limite de co-organisateurs
            can_add, error_message = event.can_add_collaborator()
            if not can_add:
            ...
            if not created and collaborator.status == 'pending':
                ...
                messages.success(
                    request,
                    _('Vous etes maintenant co-organisateur de "{event_name}"').format(
...
class TableListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    """
    Liste des tables d'un evenement avec recherche.
    """
    model = Table
    ...
    def test_func(self):
        """Verifie que l'utilisateur peut gerer l'evenement et que les tables sont activees."""
        self.event = get_object_or_404(Event, id=self.kwargs['event_id'])
        if not self.event.has_tables:
            messages.warning(
                self.request,
                _('La gestion des tables n\'est pas activee pour cet evenement.')
            )
            return False
        return user_can_manage_event(self.request.user, self.event)

    def get_queryset(self):
        """Recupere les tables avec leurs invities."""
        queryset = Table.objects.filter(
            event=self.event,
            is_deleted=False
        ).prefetch_related('guests', 'invited_guests')
    ...
    def get_context_data(self, **kwargs):
        """Ajoute les statistiques des tables."""
        context = super().get_context_data(**kwargs)
        context['event'] = self.event
        
        tables = self.get_queryset()
        total_guests = sum(table.current_guests_count for table in tables)
        total_capacity = sum(table.capacity for table in tables)
        unassigned_count = InvitedGuest.objects.filter(
            event=self.event,
            table__isnull=True,
            is_deleted=False
        ).count() + GuestResponse.objects.filter(
            event=self.event,
            table__isnull=True,
            will_attend=True,
            is_deleted=False
    ...
    def render_to_response(self, context, **response_kwargs):
        """Gere les requetes AJAX pour le chargement infini."""
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            html = render_to_string(
                'events/table_list_ajax.html',
                context,
                request=self.request
            )
            return JsonResponse({
                'html': html,
                'count': context['tables'].count(),
...
class TableCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        """Ajoute le titre et l'evenement au contexte."""
        context = super().get_context_data(**kwargs)
        context['event'] = self.event
    ...
    def form_valid(self, form):
        """Sauvegarde la table avec l'evenement et le createur."""
...
class TableUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        """Sauvegarde la table avec l'utilisateur qui modifie."""
...
class AutoAssignTablesView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, event_id):
        ...
        if result:
            ...
            messages.warning(
                request,
                _('Aucune table disponible ou aucun invite a attribuer.')
            )
...
class ExportTablesCSVView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, event_id):
        ...
        writer.writerow([
            'Table', 'Capacite', 'Occupee', 'Invite', 'Email',
            'Telephone', 'Boisson', 'Statut'
        ])
        ...
        for table in tables:
            ...
            if guests.exists():
                for guest in guests:
                    ...
                    ws.cell(row=row_idx, column=8, value='Present' if guest.will_attend else 'Absent')
                    row_idx += 1
            else:
                ws.cell(row=row_idx, column=1, value=table.id)
                ws.cell(row=row_idx, column=2, value=table.capacity)
